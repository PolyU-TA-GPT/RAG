...
if __name__ == '__main__':
    ...
    questions = Generator.strengthenUserQuestion(question,num=Rnumber,temp=0)
    ...
    for question in questions:
        ...
        for i in range(1,10,2):
            if result.find('FINAL ANSWER:')<0:
                time.sleep(5)
                result = Generator.generate(context,question,temp=i/10)
                print('retrying with temperature:',i/10)
            else:
                break
        results += [result[result.find('FINAL ANSWER:')+14:]]
        # print()
        # print('- '*40)
        # print(result)
        # print('- '*40)
    ...
    summary = Generator.AnswersToFinalAnswer(results)
    # print(summary)
