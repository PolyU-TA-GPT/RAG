...
def rephrase(question, rephrase_num):
    from langchain_openai import ChatOpenAI
    from os import getenv
    ...
    # Answer: Let's think step by step, """
    template = """
    Please provide only {rephrase_num} brief rephrased versions of the original question as the following example:
    ```
    <original question>What are available summer exchange types in PolyU?</original question>
    <rephrased question>What are PolyU available summer exchange opportunities?</rephrased question>
...
"""
    ...
    return chain.invoke({"rephrase_num":rephrase_num,"question": question}).content
