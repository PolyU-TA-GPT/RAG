...
class Retriever:
    ...
    # Please make sure that metadata_list is matched with documents_list
    # id will be created automatically as uuid v4 
    def addDocuments (self, collection_name, documents_list, metadata_list) :
        collection = self.getCollection(collection_name)

        collection.add(
            documents=documents_list,
            metadatas=metadata_list,
            ids=[uuid.uuid4() for i in range(len(documents_list)) ]
        )
