...
import chromadb
from Embedding.embedding_test import TAembedding
...
def test():
    ...
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    retriever = Retriever()
    spliter_embedder = TAembedding()
    embedder = sentenceEmbeddings()
    collection_name = retriever.createCollection("Summer Exchange")
    split_embed_result = spliter_embedder.split_pdf(file_path="{}/assets/Summer_Outbound_Info_Session.pdf".format(cur_dir))
    num = len(split_embed_result)
    embeddings_list = [i["embedding"] for i in split_embed_result]
    documents_list = [i["sentence"] for i in split_embed_result]
    ...
    query_result = retriever.query(collection_name = collection_name, query_embeddings= ["This is an example sentence", "Each sentence is converted to a vector"])
