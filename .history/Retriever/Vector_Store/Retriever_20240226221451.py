...
def test():
    ...
    result = spliter_embedder.split_pdf(file_path="{}/assets/Summer_Outbound_Info_Session.pdf".format(cur_dir))
    num = len(result)
    embeddings_list = [i["embedding"] for i in result]
    documents_list = [i["sentence"] for i in result]
    ...
    retriever.addDocuments(collection_name=collection_name, embeddings_list=embeddings_list, \
                           documents_list=documents_list, metadata_list=[])
