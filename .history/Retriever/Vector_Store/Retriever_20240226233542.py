...
class Retriever:
    ...
    def addDocuments (self, collection_name: str, embeddings_list: list[list[float]], documents_list: list[dict], metadata_list: list[dict]) :
        ...
        with open ("{:0}/assets/log/{:1}.json".format(self.cur_dir, collection_name), "r+") as chunklog:
...
def load_split_pdf(filepath: str) -> list:
...
def test():
    ...
    embed_result = embed_result.tolist() # tensor to list
